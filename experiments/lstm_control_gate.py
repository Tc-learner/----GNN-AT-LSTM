"""
LSTM with Adaptive Forget Gate + Extra Control Gate (PyTorch)

自定义 LSTM 变体：遗忘门和额外的控制门同时融入输入与期望值的差异，
增强模型对异常输入的鲁棒性。使用合成含单点异常的序列进行测试。
"""
import numpy as np
import torch
...
class LSTMCellWithAdaptiveForgetAndControlGates(nn.Module):
    """带有自适应遗忘门和额外控制门的 LSTM 单元"""

    def __init__(self, input_dim, units):
        super().__init__()
        dim_concat = input_dim + units + input_dim  # inputs + hidden + input_diff
        self.forget_gate = nn.Linear(dim_concat, units)
        self.control_gate = nn.Linear(dim_concat, units)
    ...
    def forward(self, x, state):
        ...
        control = torch.sigmoid(self.control_gate(concat_gates))

        c = c_prev * (forget + control) + \
...
class CustomLSTMWithAdaptiveForgetAndControlGates(nn.Module):
    """在时间维度上循环执行自定义 LSTM 单元"""

    def __init__(self, input_dim, units):
        super().__init__()
        self.cell = LSTMCellWithAdaptiveForgetAndControlGates(input_dim, units)
        self.units = units
...
if __name__ == "__main__":
    ...
    model = nn.Sequential(
        CustomLSTMWithAdaptiveForgetAndControlGates(input_dim=1, units=8),
        nn.Linear(8, 1)
    )
    ...
    print("Predicted Data (LSTM with Adaptive Forget and Control Gates):")
